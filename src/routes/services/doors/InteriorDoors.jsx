...
const images = import.meta.glob(
  "../../../assets/images/services/doors/interior-doors/*.{png,jpg,jpeg,webp}",
  { eager: true }
);
const intDoors = Object.values(images).map((img) => img.default);
...
const InteriorDoors = () => {
  ...
  const handleNext = () => setCurrent((prev) => (prev + 1) % intDoors.length);
  const handlePrev = () =>
    setCurrent((prev) => (prev - 1 + intDoors.length) % intDoors.length);
    ...
    <div className="bg-globalColor5 py-10">
      ...
      >
        <div className="main-heading col-span-full mb-8 w-fit mx-auto flex flex-col gap-1.5">
          <h2 className="text-4xl text-globalColor0">{t("interiorDoors")}</h2>
        </div>
        {intDoors.map((imgSrc, index) => (
          <SingleDecor
            key={index}
            imgSrc={imgSrc}
            setCurrent={() => {
