...
const images = import.meta.glob(
  "../../../assets/images/services/doors/palace-residential/*.{png,jpg,jpeg,webp}",
  { eager: true }
);
const PalaceAndResidentialImgs = Object.values(images).map(
  (img) => img.default
);
...
const PalaceAndResidential = () => {
  ...
  const handleNext = () =>
    setCurrent((prev) => (prev + 1) % PalaceAndResidentialImgs.length);
  const handlePrev = () =>
    setCurrent(
      (prev) =>
        (prev - 1 + PalaceAndResidentialImgs.length) %
        PalaceAndResidentialImgs.length
    );
  ...
  return (
    <div className="bg-globalColor5 py-10 select-none">
      ...
      >
        ...
        {PalaceAndResidentialImgs.map((imgSrc, index) => (
          <SingleDecor
            key={index}
            imgSrc={imgSrc}
            setCurrent={() => {
        ...
        {showModal && (
          <Lightbox
            current={current}
            images={PalaceAndResidentialImgs}
            imgRef={imgRef}
